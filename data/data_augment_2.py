...
# h_new, w_new = 480, 480 
ratio_scale = 2
h_new, w_new = 360*ratio_scale, 640*ratio_scale
...
def _pad_to_rectangle_random_position(image, rgb_mean, boxes, labels, landm, pad_image_flag):
    ...
    landm[:, 1::2] += offset_Y
    try:
        image_t[offset_Y:offset_Y + height, offset_X:offset_X + width] = image
    except Exception as e:
        interp_methods = [cv2.INTER_LINEAR, cv2.INTER_CUBIC, cv2.INTER_AREA, cv2.INTER_NEAREST, cv2.INTER_LANCZOS4]
        interp_method = interp_methods[random.randrange(5)]
        image_t = cv2.resize(image, (w_new_tmp, h_new_tmp), interpolation=interp_method) # (w, h)
...
def scale_camera(image_t, boxes_t, labels_t, landm_t, pad_image_flag):
    b_w_t = (boxes_t[:, 2] - boxes_t[:, 0])
    b_h_t = (boxes_t[:, 3] - boxes_t[:, 1])
    scale = 1
    ...
    boxes_t /= (scale*ratio_scale)
    landm_t /= (scale*ratio_scale)
    return image_t, boxes_t, labels_t, landm_t, pad_image_flag

class preproc2(object):
    ...
    def __call__(self, image, targets):
        ...
        image_t, boxes_t, labels_t, landm_t, pad_image_flag = scale_camera(image_t, boxes_t, labels_t, landm_t, pad_image_flag)
